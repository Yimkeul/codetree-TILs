...
func Q_자동잔기깎이() {
    ...
    func minimumGrassRegrowCycle(n: Int, movements: [(String, Int)]) -> Int {
        var visitedPositions: [((Int, Int), Int)] = [] // 위치와 도착 시간을 기록
        ...
        for (direction, distance) in movements {
            ...
            for _ in 0..<distance {
                ...
                if let lastVisit = visitedPositions.first(where: { $0.0 == currentPosition })?.1 {
                    // 방문 주기 계산
                ...
                // 위치와 현재 시간을 기록 (기록 갱신)
                visitedPositions.append((currentPosition, time))
            }
        }
